...
function handleClick(clickedButtonEvent) {
    ...
    /*решил по хулиганить и сделать по своему*/
    
    let text = 'Всякий текст';
    if (clickedButtonEvent.target.innerText == "Подробнее") { 
        clickedButtonEvent.target.innerText = "Отмена";
        clickedButtonEvent.target.parentNode.children[1].style.display = 'none';
        clickedButtonEvent.target.insertAdjacentHTML('beforebegin', `<div class="desc">${text}</div>`);
    } else if (clickedButtonEvent.target.innerText == "Отмена") { 
        clickedButtonEvent.target.innerText = "Подробнее";
        clickedButtonEvent.target.parentNode.children[1].style.display = 'block';
